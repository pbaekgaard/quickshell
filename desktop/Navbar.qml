...
Item {
    ...
    // ---------- AI toggle (hosts-based LLM blocker) ----------
    property string aiGlyph:   "󰚩"
    property string aiTooltip: "LLM Access: Active"
    ...
    // Initial fetch + half-hourly refresh.
    Timer {
        interval: 1800000
        running: true
        repeat: true
    ...
    Process {
        ...
        running: false
        command: ["bash", "-lc",
            "$HOME/.config/quickshell/scripts/toggle_ai.py --status"]
        stdout: StdioCollector {
            onStreamFinished: {
                const raw = this.text.trim();
                try {
                    const d = JSON.parse(raw);
                    if (root.aiGlyph !== d.text)   root.aiGlyph   = d.text;
                    if (root.aiTooltip !== d.tooltip) root.aiTooltip = d.tooltip;
                } catch (_) {}
            }
        }
    }
    Timer { interval: 5000; running: true; repeat: true; triggeredOnStart: true
        onTriggered: { aiProbe.running = false; aiProbe.running = true; } }

    // ---------- AI blocker toggle (dedicated so onExited can refresh) ----------
    Process {
        id: aiToggler
        running: false
        command: ["bash", "-lc",
            "sudo $HOME/.config/quickshell/scripts/toggle_ai.py --toggle"]
        onExited: {
    ...
    function toggleAi() {
        aiToggler.running = false;
        aiToggler.running = true;
    }
